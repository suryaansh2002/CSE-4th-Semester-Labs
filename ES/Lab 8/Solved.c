...
int main(void) {
    unsigned char Msg1[10] = {"Suryaansh"};
    unsigned char Msg2[11] = {"Rathinam"};
...
void lcd_init()
{
    ...
    lcd_comdata(0x28, 0);  // function set to 4 bit 
	delay_lcd(30000);
...
void write(int temp2, int type)  // write to command/data reg
{
    clear_ports();
    LPC_GPIO0->FIOPIN =
        temp2;  // Assign the value to the data lines
	if(type==0)
...
void clear_ports(void) {
    ...
    LPC_GPIO0->FIOCLR =0x0F << 23;  // Clearing data lines 
		LPC_GPIO0->FIOCLR = 1<<27; //Clearing RS
